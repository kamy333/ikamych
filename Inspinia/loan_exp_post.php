...
if (User::is_caroline() || User::is_weslley()) {
    ...
    $urla = $url . "?sort=ASC";
        ...
        if (isset($_GET["type_category"])) {
            ...
            $cat_name = MyExpenseMumPost::get_category_name($cat);
    ...
    $sum = MyExpenseMumPost::sum_field_where_by_sql($sql);

    echo "<h1 >Mum Post finance</h1>";

//    echo "<a href='/Inspinia/loan_exp_1.php' class='button'>Summary Mum</a>";

    if ($sum < 0) {
        $sum = "<span style='color: red'><b>CHF " . number_format($sum, 2) . "</b></span>";
        $due = "<span style='color: red'><b>$cat_name : Total payment PostFinance :</b></span>";
    } else {
        $sum = "<span style='color: blue'><b>CHF " . number_format($sum, 2) . "</b></span>";
        $due = "<span style='color: blue'><b> $cat_name : Total payment PostFinance :</b></span>";
    }

    $msg .= "<br>";

    $msg .= "<div style='font-size: small'>";
    $msg .= MyExpenseMumPost::form_select_person();
    $msg .= "</div>";
    $msg .= str_repeat("&nbsp;", 5) . $due . str_repeat("&nbsp;", 5) . $sum;
        if (User::is_admin()) {
            $msg .= str_repeat("&nbsp;", 10) . "<a href='/public/admin/crud/ajax/manage_ajax.php?class_name=MyExpenseMumPost'><span style='color:blueviolet;'><b>Add Expense Item</b></span></a>";
        }
    ...
    $title = "$person - Kamran ";;
    echo Table::ibox_table(MyExpenseMumPost::aPerson($p_id, true, $exclude, $sort, $show_doc, false), $title, 12, 0);
