<?php
// Original location: public/_f/kamy/recurring_appointment_email.php
$IS_PRODUCTION = true;
?>
...
if ($IS_PRODUCTION) {
    $providedCode = isset($_GET["code"]) ? (string)$_GET["code"] : '';
    if ($providedCode !== '' && hash_equals((string)CODE_CALENDAR, $providedCode)) {
...
//    $dayNumberNew = $_GET['dayNumber'] ?? ($dayNumber + 3);
    $dayReportNew = isset($_GET['dayReport']) ? (int)$_GET['dayReport'] : $dayReport;
    $dayReportNew = max(1, min(31, $dayReportNew));
...
//        global $session;
        ...
        $error = "Date Malformed String Exception: " . $e->getMessage();
        sendErrorEmail($error);
        $msg .= "<li style='color: red'>" . h($error) . "</li>";
        return;
...
//    echo "<br>" . $message . "<br>";
    ...
    try {
        if (!$mail->send()) {
            $error = "Medical certificate email could not be sent.";
            sendErrorEmail($error);
            $msg .= "<li style='color: red'>" . h($error) . "</li>";
        } else {
...
$dayNumber = isset($_GET['dayNumber']) ? (int)$_GET['dayNumber'] : 15;
$dayNumber = max(1, min(31, $dayNumber));
$dayReport = isset($_GET['dayReport']) ? (int)$_GET['dayReport'] : $dayNumber + 3;
$dayReport = max(1, min(31, $dayReport));
$getHourStart = $_GET['hourStart'] ?? null;
