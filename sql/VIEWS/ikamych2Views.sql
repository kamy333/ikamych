...
(SELECT DISTINCT c.Pseudo AS pseudo,
                 c.Depart AS adresse
...
(SELECT DISTINCT c.Pseudo  AS pseudo,
 ...
 FROM DatabaseCourse AS c)
UNION
(SELECT DISTINCT t.pseudo AS pseudo,
                 t.depart AS adresse
...
(
select year(`mycigarette`.`cig_date`)      AS `year`,
...
(
select year(`c`.`cig_date`)                                            AS `year`,
       month(`c`.`cig_date`)                                           AS `monthno`,
       concat_ws(' ', monthname(`c`.`cig_date`), year(`c`.`cig_date`)) AS `month`,
       sum(`c`.`number_cig`)                                           AS `total`
from `mycigarette` `c`
group by year(`c`.`cig_date`) desc, concat_ws(' ', monthname(`c`.`cig_date`), year(`c`.`cig_date`)) desc);
...
(
select year(`mycigarette`.`cig_date`) AS `year`, sum(`mycigarette`.`number_cig`) AS `total`
from `mycigarette`
...
(
select `transport_model`.`heure`         AS `heure`,
...
(
select `transport_model_visible_no`.`heure`         AS `heure`,
...
(
select `transport_model_visible_yes`.`heure`         AS `heure`,
...
select distinct `transport_programming`.
        ...
        ,
                ...
                (case
                     ...
                     when ((to_days(`transport_programming`.`course_date`) - to_days(now())) = 1)
                         then 'demain'
                     when ((to_days(`transport_programming`.`course_date`) - to_days(now())) < 0)
                         then concat('il y a ', -((to_days(`transport_programming`.`course_date`) - to_days(now()))),
                                     ' jours')
                     when ((to_days(`transport_programming`.`course_date`) - to_days(now())) > 0)
                         then concat('dans ', (to_days(`transport_programming`.`course_date`) - to_days(now())),
                                     ' jours')
                     when ((to_days(`transport_programming`.`course_date`) - to_days(now())) = 0)
                         then 'aujourd\'hui'
                     else 'now' end)                                                                         AS `str_time_fr`
        ,
                date_format(`transport_programming`.`course_date`,
                            get_format(DATE, 'EUR'))                                                         AS `date_format`,
                ...
                monthname(`transport_programming`.`course_date`)                                             AS `monthname`,
                year(`transport_programming`.`course_date`)                                                  AS `year`,
                week(`transport_programming`.`course_date`, 0)                                               AS `week`,
                count(`transport_programming`.`course_date`)                                                 AS `total_course`,
                ...
                sum(((`transport_programming`.`chauffeur_id` = '') or
                     isnull(`transport_programming`.`chauffeur_id`)))                                        AS `erreur_chauffeur`,
                sum(((`transport_programming`.`depart` = '') or isnull(`transport_programming`.`depart`) or
                     (`transport_programming`.`arrivee` = '') or
                     isnull(`transport_programming`.`arrivee`)))                                             AS `erreur_address`,
                sum(((`transport_programming`.`pseudo` = 'autres') or
                     ((`transport_programming`.`pseudo` = 'colline') and
                      ((`transport_programming`.`pseudo_autres` = '') or
                       isnull(`transport_programming`.`pseudo_autres`)))))                                   AS `erreur_autres`,
                ...
                sum(((`transport_programming`.`depart` = '')
                    ...
                    or ((`transport_programming`.`pseudo` = 'carouge_sang')
                        and ((`transport_programming`.`bon_no` = '')
                            or
                             isnull(`transport_programming`.`bon_no`)))))                                    AS `erreur_general`
