...
class LinksCategoryVisibility extends DatabaseObject
{
    ...
    protected static $db_fields = ['id', 'source_field', 'source_value', 'hidden', 'username'];

    public static $required_fields = ['source_field', 'source_value', 'hidden'];

    protected static $db_fields_table_display_short = ['id', 'source_field', 'source_value', 'hidden'];
    protected static $db_fields_table_display_full = ['id', 'source_field', 'source_value', 'hidden', 'username'];
    protected static $db_field_exclude_table_display_sort = null;

    public static $fields_numeric = ['id', 'hidden'];
    public static $get_form_element = ['source_field', 'source_value', 'hidden'];
    public static $get_form_element_others = [];
    public static $form_default_value = ['source_field' => 'category', 'hidden' => '1'];
    ...
    public static $db_field_search = ['search_all', 'source_field', 'source_value', 'hidden'];
    ...
    public static function handle_public_request()
    {
        ...
        static::ensure_table();
        static::save_visibility($_POST['all_values'] ?? [], $_POST['visible_values'] ?? []);
    ...
    private static function save_visibility(array $all_values, array $visible_values)
    {
        ...
        foreach ($all as $item) {
            ...
            $database->execute_prepared(
                "INSERT INTO " . static::$table_name . " (source_field, source_value, hidden, username) VALUES (?, ?, 1, ?)",
                [$item['source_field'], $item['source_value'], $username],
                "sss"
            );
        }
    }
